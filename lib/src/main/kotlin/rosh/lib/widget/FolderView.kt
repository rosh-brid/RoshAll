...
class FolderView @JvmOverloads constructor(
    ...
    init {
        ...
        emptyView = TextView(context).apply {
            ...
            textSize = 14f
            setTextColor(context.getColor(rosh.lib.R.color.tulisan))
        ...
        inner class TreeViewHolder(
            ...
            fun bind(node: FileNode, isSelected: Boolean) {
                ...
                if (node.file.isDirectory) {
                    ...
                    icon.setImageResource(rosh.lib.R.drawable.folder)
                    indicator.text = if (node.isExpanded) "▼" else "▶"
                    title.setTypeface(null, Typeface.BOLD)
                } else {
                    val act = hostActivity
                    if (act != null) {
                        val ikon = rosh.view.ikon.IkonFile(act)
                        ikon.setGlide(icon)
                        icon.setImageResource(ikon.Tipe(node.file))
                    }
                    indicator.text = ""
                ...
                itemView.setBackgroundColor(
                    if (isSelected) context.getColor(rosh.lib.R.color.ui_biru) else Color.TRANSPARENT
                )
            }
        }
    }
